#' reviewresults_req_reports
#'
#' gets a review results for reports/full texts from the rayyan API and outputs the results an R object
#'
...
reviewresults_req_reports <- function(api_tokens, review_id) {
  ...
  limbs <- unlist(body[[1]])
  filter_key <- limbs[names(limbs)=='filter_key']
  
  reviews_route <- paste0("/api/v1/reviews/", review_id, "/results/?extra[stages]=", filter_key)
